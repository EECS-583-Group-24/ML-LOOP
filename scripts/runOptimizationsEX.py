...
def compile_test_file_with_optimization(input_file, opt_sequence,test_directory,id, non_optimized_file, output_dir,n):
    ...
    optimized_file = os.path.join(output_dir, f"{os.path.splitext(input_file)[0]}_{id}.ll")
    ...
    executable_file = os.path.join(output_dir, f"{os.path.splitext(input_file)[0]}_{id}")
...
def createDataset(directory,d,start):
    ...
    for input_file in test_files:
        ...
        if(store_size==1):
            ...
            print(optimization_permutations.index(opts[0]))
        else:
