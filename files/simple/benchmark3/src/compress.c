 ...
 * 9/4/13 ajamshid: revisions to fix modern compilation errors and warnings
 */
 ...
 *
 * $Header: /z/repository/benchmarks/specint92/026.compress/src/compress.c,v 1.2 2005/01/04 22:46:33 mchu Exp $
 * $Log: compress.c,v $
 * Revision 1.2  2005/01/04 22:46:33  mchu
 * changed source to define parameters of rindex() if IMPACT_FIX is set
 *
 * Revision 1.1  2002/10/10 22:53:43  ntclark
 * added the source and set it up.
 *
 * Revision 4.0  2001/12/08 00:43:31  sias
 * Manual merge after loss of repository -- JWS 20011207
 *
 * Revision 1.1.1.1  2000/01/14 16:35:08  merten
 * Import the bench_info directories.
 *
 ...
 *
 */
static char rcs_ident[] = "$Header: /z/repository/benchmarks/specint92/026.compress/src/compress.c,v 1.2 2005/01/04 22:46:33 mchu Exp $";
...
int scott_hack(code_int *i, long fcode, int disp, int hsize_reg, code_int *ent);
...
void compress() {
	...
	if ( (long)htabof (i) > 0 )  {
            int c = 0;
	    c = scott_hack(&i, fcode, disp, hsize_reg, &ent);
	    if (c) continue;
        }
...
//    time_t timep[2];
	...
	printf("Here is the mode: %d\n", mode++);
    fclose(stdout);
	printf("Here is the mode incremented: %d\n", mode++);
...
void oops (int a )	/* wild pointer -- assume bad input */
{
    if ( do_decomp == 1 ) 
