...
for filename in *.c *.cpp; do
    if [ -f "$filename" ]; then
        ...
        echo "Output for $filename:" >> paper_features.csv
        opt --disable-output -load-pass-plugin="${PATH2LIB}" -passes="${PASS}" "${filename%.*}.profdata.bc" >> paper_features.csv 2>&1
        echo "" >> paper_features.csv  # Add an empty line for better readability
