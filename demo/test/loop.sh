...
for filename in *.c *.cpp; do
    if [ -f "$filename" ]; then
        ...
        opt --disable-output -load-pass-plugin="${PATH2LIB}" -passes="${PASS}" "${filename%.*}.profdata.bc" >> loop_features.csv 2>&1
        echo "" >> loop_features.csv  # Add an empty line for better readability
